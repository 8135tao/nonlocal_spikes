...
\begin{Remark}[normalizing second moments]\label{r:rs} There exists a coordinate change $x=T_0y$ such that $\tilde{\K}(y):=|\mathrm{det}\,T_0|\K(T_0y)$ satisfies
\[
...
\textcolor{red}{Comment on the assumption $n<6$: The assumption $n<6$ is a technical one, which comes from the fact that the ground states for the semilinear equation $\Delta w - w + w^p = 0$ only exists for $1<p<\frac{n+2}{n-2}$. (\cite[Lem. 13.3]{gs})}
...
\end{pmatrix},
\]
where terms with subscript $\mathrm{cc}$ denote a scalar, terms with subscript $\mathrm{ch}$ a $(k-1)$ dimensional row vector, terms with subscript $\mathrm{hc}$ a $(k-1)$ dimensional column vector, and terms with subscript $\mathrm{hh}$ a $(k-1)\times (k-1)$ matrix. We then have that $\widehat{L}_\mathrm{cc},\widehat{L}_\mathrm{ch},\textcolor{red}{\widehat{L}_\mathrm{hh}}$ are continuous and bounded in $\xi$, and $\widehat{L}_\mathrm{hc} \in L^\infty(\R^n, \textcolor{red}{\R^{k-1}})$.
\end{Lemma}
\begin{Proof}
